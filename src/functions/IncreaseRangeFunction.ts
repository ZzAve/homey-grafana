 ...
 *
 */
class IncreaseRangeFactory implements QueryableFunctionFactory {
    hasMatchingSyntax: ((query: any) => boolean) = (query: any) => query.startsWith("increaseRange(")
...
export const increaseRangeFactory = new IncreaseRangeFactory();

class IncreaseRange implements QueryableFunction {
    private _query: any;
    ...
    async apply(subQueryResolver: any) {
        ...
        debug(`Applying increaseRange for results of subQuery resolver. targets: ${result.map((it: any) => it.target)} `)
        return this.applyIncreaseFunctionByFirstEntry(result)
    }

    applyIncreaseFunctionByFirstEntry(result: any) {
        return result.map((entry: any) => {
                // get base value:
                const referenceDatapoint = entry.datapoints.find((it: any) => it[0] != null);
                return {
                    target: entry.target,
                    datapoints: entry.datapoints
                        .map((it: any) => it[0] === null ? null : [it[0] - referenceDatapoint[0], it[1]])
